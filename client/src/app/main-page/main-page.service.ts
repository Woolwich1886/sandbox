...
import { UserSelectionDialogComponent } from '../material/user-selection-dialog/user-selection-dialog.component';
...
export class MainPageService {
  ...
  getNavs(): NavCard[] {
    return [
      { label: 'Сферы', description: 'RxJs, NgRx', callback: () => this.callbackSphere() },
      { label: 'Мессенджер', description: 'Веб-сокеты', callback: () => this.callbackMessenger() },
    ];
  }
  ...
  private callbackMessenger(): void {
    lastValueFrom(this.httpClient.request<UserInfo[]>('GET', SERVER + '/rest/users'))
      ...
      .then(ref => firstValueFrom(ref.afterClosed()).then(id => this.router.navigateByUrl(`messenger/user/${id}/chat`)));
  }
